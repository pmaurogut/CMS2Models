...
models_to_fit<-expand.grid(Response=Variables_to_model,
...
Fit_ALL<-ddply(models_to_fit,c("Response","Lidar","Region"),
		function(x,y){
			...
			print(x$Lidar)
			if(x$Lidar=="Both"){
				Predictors<-c(y$columns$Lidar_ALL,
						y$columns$Lidar_FIRST,
						y$columns$Climate,
						y$columns$Topo,"L3_KEY")
			}else{
				Predictors<-c(y$columns[[x$Lidar[1]]],
						y$columns$Climate,
						y$columns$Topo,"L3_KEY")
			}
			print(x$Response)
...
VSURF_ALL<-ddply(Fit_ALL,c("Response","Lidar","Region"),function(x){
		...
		output_VSURF<-paste("Output/Models_VSURF/",x$Response[1],
				"_",x$Lidar[1],"_",x$Region[1],".Rdata",sep="")
...
VSURF_ALL<-ddply(VSURF_ALL,c("Response","Lidar","Region"),function(x){
			...
			a<-try({
				...
				CV_intrep<-CV_pred<-data.frame(obs=data[,1],
						pred=NA,fold=NA)
